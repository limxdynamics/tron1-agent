...
from bailing.utils.utils import read_json_file, write_json_file
    ...
    def read_dialogue_file(file_path):
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            try:
                ...
                logger.error(f"An error occurred when parsing JSON:{e}")
                return []
    ...
    def read_dialogues_in_order(self, directory):
        ...
        for file_path in files:
            if file_path in self.memory["history_memory_file"]:
                logger.info(f"{file_path}: The history of conversations has formed memory")
                continue
            logger.info(f"propossing: {file_path}")
